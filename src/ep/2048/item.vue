...
import type { TileItemPresets } from "./use2048"

const props = defineProps<{
  level: number
  preset: TileItemPresets
}>()

const animationName = ref<"tile-merge" | "tile-popup">()

watch(
  () => props.level,
  (_, oldValue) => {
    animationName.value = oldValue ? "tile-merge" : "tile-popup"
  },
  {
    immediate: true,
  }
...
<template>
  <div class="absolute rounded transition">
    <div
      class="flex h-full w-full items-center justify-center rounded text-3xl font-bold"
      :class="[animationName, preset.getTileColor(level)]"
      @animationend="animationName = undefined"
    >
      {{ preset.getTileLabel(level) }}
    </div>
  </div>
</template>
