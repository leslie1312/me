...
export class Model {
  ...
  init(args?: Level | BoardMeta): void {
    ...
    this.initTimer()
    this.flags.length = 0
    this.grids = Array.from({ length: this.board.w * this.board.h }, (_, uid) => ({ uid }))
  }
  ...
  load(options: StorageOptions) {
    ...
    this.board = board
    this.flags.length = 0
    this.grids = Array.from({ length: this.board.w * this.board.h }, (_, uid) => ({
      uid,
      open: false,
    }))
    grids.forEach(item => {
  ...
  doVictoryJudgement(mode: "open" | "flag") {
    ...
    if (mode == "open") {
      ...
      victory = count === this.flags.length && this.flags.every(uid => this.grids[uid].mine)
    }
    if (victory) {
      this.state = "victory"
      this.openAll()
  ...
  getSiblings(grid: GridMeta, mode?: "mine" | "hide"): GridMeta[] {
    ...
    for (let i = 0; i < vec.length; i++) {
      ...
      if (mode == "mine" && !grid.mine) {
        continue
      }
      if (mode == "hide" && grid.open) {
        continue
      }
      result.push(grid)
    }
